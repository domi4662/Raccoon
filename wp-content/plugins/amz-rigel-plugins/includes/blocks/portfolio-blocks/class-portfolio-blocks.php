<?php
    class portfolio_blocks {
        ...
        public function build_portfolio_blocks( $block = 'portfolio_block1', $options = array(), $post_count, $total_post ) {
            require_once( RIGEL_BLOCKS_DIR. '/portfolio-blocks/blocks/portfolio-block.php' );
        ...
        public function portfolio_modules( $style = 'style1', $options, $size ) {
            require_once( RIGEL_BLOCKS_DIR. '/portfolio-blocks/style/'. $style .'.php' );
        ...
        public function filter( $show_filter = 'yes' ) {
        ...
        public function pagination( $args = array(), $values = array() ) {
            ...
            if( 'load_more' == $values['style'] ) {
                ...
                $output .= "<div id='block-load-more-btn' class='block-load-more-btn pix_button clear button-center'>
            ...
            elseif( 'autoload' == $values['style'] ) {
                $output .= "<div id='block-load-more-btn' class='block-load-more-btn amz-autoload pix_button clear button-center'>
                    <a href='#' data-paged='". esc_attr( $paged ) ."' data-args='". json_encode( $args ) ."' data-values='". json_encode( $values ) ."'>". esc_html( $values['loadmore_text'] ) ."</a>
                    <div class='spinner' style='display: none;'>
                        <div class='spinner-inner'>
                            <div class='double-bounce1'></div>
        ...
        public function get_image_thumb( $post_id, $block = 'portfolio_block1', $post_count ) {
            ...
            //Get Porfolio Image
            $gallery_json = rigel_get_meta_value( $post_id, '_amz_gallery' );
            $gallery = json_decode( $gallery_json, true );
            $thumb = !empty( $gallery ) ? $this->get_image_by_id( $size['width'], $size['height'], $gallery[0]['itemId'], 1, 1, 1 ) : '';
    ...
    require_once( RIGEL_BLOCKS_DIR .          '/portfolio-blocks/portfolio-block-shortcode.php' );

    // Portfolio Block Extend Vc
    require_once( RIGEL_BLOCKS_DIR .          '/portfolio-blocks/portfolio-block-vc.php' );

    // Ajax Portfolio Block
    require_once( RIGEL_BLOCKS_DIR .          '/portfolio-blocks/portfolio-block-ajax.php' );