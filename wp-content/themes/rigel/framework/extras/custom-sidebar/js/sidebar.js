...
(function($){
	...
	PixSidebar.prototype = {

		create_form: function(){
			this.widget_wrap.append(this.widget_add.html());
			this.widget_name = this.widget_wrap.find('input[name="pix-sidebar-widgets"]');
			this.nonce = this.widget_wrap.find('input[name="pix-delete-sidebar"]').val();   
		},

		add_del_button: function(){
			this.widget_area.find('.sidebar-pix-custom').append('<span class="pix-delete"></span>');
		},

		bind_events: function(){
			this.widget_wrap.on('click', '.pix-delete', $.proxy( this.delete_sidebar, this));
		},
		...
		delete_sidebar: function(e){
			...
			$.ajax({
				...
				url: window.ajaxurl,
				data: {
					action: 'rigel_ajax_delete_custom_sidebar',
					name: widget_name,
					_wpnonce: obj.nonce
				},
