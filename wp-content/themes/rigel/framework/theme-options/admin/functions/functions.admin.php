...
function generate_options_css($newdata) {
	...
	$css_dir = RIGEL_ADMIN . '/_css/';
	
	/** Save on different directory if on multisite **/
	if(is_multisite()) {
		$pix_uploads_dir = trailingslashit($uploads['basedir']);
	} else {
		
		$file_dir = get_template_directory() . '/_css/';
		@chmod( $css_dir, 0755 );
		@chmod( $css_dir . 'dynamic-css.php', 0664 );
		@chmod( $file_dir, 0755);

		$pix_uploads_dir = $file_dir;
	}
	...
	if ( ! $wp_filesystem->put_contents( $pix_uploads_dir . 'custom.css', $css, 0644 ) ) {
	    return true;
	}
