...
function optionsframework_add_admin() {
    ...
    $of_page = add_theme_page( RIGELNAME, 'Theme Options', 'edit_theme_options', 'optionsframework', 'optionsframework_options_page');
...
function optionsframework_options_page(){
	...
	*/	
	...
	include_once( RIGEL_ADMIN_OPTION . '/admin/front-end/options.php' );
...
function of_style_only(){
	...
	wp_enqueue_style( 'rigel-themeoptions', ADMIN_DIR . 'assets/css/fonts.css', array(), '1.0', 'all' );
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'import_options'){
		...
		$smof_data = json_decode( stripslashes_deep( $_POST['data'] ), true); //100% safe - ignore theme check nag
		of_save_options($smof_data);
