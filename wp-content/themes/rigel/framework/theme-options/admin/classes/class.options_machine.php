		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
					...
					foreach ( $demos as $key => $demo ) {
						...
						$output .= '<div class="pix-demo-button">
								  ...
								  <p class="import-messages">
								  	<span class="info msg"><i class="icon-pix-fonts"></i>'.  esc_html__("Please wait, Don't reload this page. It will take some time to complete depends on your internet connection! Once done, You will be notified.", "rigel") .'</span>
									<span class="success msg"><i class="icon-check-mark"></i>'.  esc_html__("Demo Data Successfully imported!", "rigel") .'</span>
									<span class="import-error"><i class="icon-remove"></i>'.  esc_html__("Data not Imported, Please reload the page and try again. <br><br> <strong>Note:</strong>If you continously receive error message then please try to set file permission to cmod 777 (for both demo and demo.xml in \"demo_data/demo.xml\"). If it still doesn't work, please upload demo xml using wordpress importer. Demo xml file should be available in theme package", "rigel") .'</span>
								  </p>';
					}
				...
				case 'custom_fonts':
					...
					if ( $fonts ) {
						...
						foreach ($fonts as $font) {
							...
							$i++;
							$order = $i;
							$output .= Options_Machine::optionsframework_fonts_function($value['id'],$value['std'],$oldorder,$order);
						}
					}			
					$output .= '</ul>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ', 'rigel' ).'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'select_google_font':
					...
					if(isset($g_typography_stored['g_face'])) {
						...
						if( $custom_fonts ) {
							$output .= '<optgroup class="user-fonts" label="'. __('My Fonts', 'rigel') .'">';
							...
							$output .= '</optgroup>';
							$output .= '<optgroup class="google-webfonts" label="'. __('Google Web Fonts', 'rigel') .'">';
						}
