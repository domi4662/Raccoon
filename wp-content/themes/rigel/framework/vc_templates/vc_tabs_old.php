...
foreach ( $tab_titles as $tab ) {
    ...
    if(isset($tab_matches[1][0])) {
        ...
        $icons = '';
        if( $tab_icon[1][0] != '' ){
            $icons ='<i class="pix '. esc_attr( $tab_icon[1][0] ) .'"></i>';
        }
        //var_dump($icons);
...
$output .= "\n\t".'<div class="'. esc_attr( $css_class.' '. $style .' '. $align .' '. $side ) .'" data-interval="'. esc_attr( $interval ) .'">';
...
if ( 'vc_tour' == $this->shortcode) {
    $output .= "\n\t\t\t" . '<div class="wpb_tour_next_prev_nav vc_clearfix"> <span class="wpb_prev_slide"><a href="#prev" title="'. esc_attr__( 'Previous slide', 'rigel' ) .'">'. esc_html__( 'Previous slide', 'rigel' ).'</a></span> <span class="wpb_next_slide"><a href="#next" title="'. esc_attr__( 'Next slide', 'rigel' ) .'">'. esc_html__( 'Next slide', 'rigel' ) .'</a></span></div>';
}
